import random
import yfinance as yf
import pandas as pd
import numpy as np
from sklearn.ensemble import RandomForestClassifier, StackingClassifier
from xgboost import XGBClassifier
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import precision_score
from datetime import timedelta
import torch
import torch.nn as nn
from typing import Tuple, List, Any, Optional

# --- Deep Learning Models ---
class LSTMClassifier(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 64, num_layers: int = 2):
        super(LSTMClassifier, self).__init__()
        self.lstm = nn.LSTM(input_dim, hidden_dim, num_layers, batch_first=True, dropout=0.2)
        self.fc = nn.Linear(hidden_dim, 1)
        self.sigmoid = nn.Sigmoid()
        
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out, _ = self.lstm(x)
        out = out[:, -1, :] 
        out = self.fc(out)
        return self.sigmoid(out)

class GRUClassifier(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 64, num_layers: int = 2):
        super(GRUClassifier, self).__init__()
        self.gru = nn.GRU(input_dim, hidden_dim, num_layers, batch_first=True, dropout=0.2)
        self.fc = nn.Linear(hidden_dim, 1)
        self.sigmoid = nn.Sigmoid()
        
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out, _ = self.gru(x)
        out = out[:, -1, :] 
        out = self.fc(out)
        return self.sigmoid(out)

# --- Utilities ---

def set_global_seed(seed: int = 42) -> None:
    """Sets seeds for all libraries to ensure deterministic results."""
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed)

def fetch_data(ticker: str, start: str = "2020-01-01", end: str = "2024-01-01", interval: str = "1d") -> pd.DataFrame:
    """
    Fetches historical stock data from Yahoo Finance with robust error handling.
    """
    # API Constraints Correction
    now = pd.Timestamp.now()
    if interval == "1h":
        max_start = now - timedelta(days=729)
        if pd.to_datetime(start) < max_start:
            start = max_start.strftime('%Y-%m-%d')
    elif interval == "1m":
        max_start = now - timedelta(days=6) 
        if pd.to_datetime(start) < max_start:
            start = max_start.strftime('%Y-%m-%d')
            
    try:
        # Improved download attempt
        df = yf.download(ticker, start=start, end=end, interval=interval, auto_adjust=True, progress=False)
        
        if df.empty:
             # Retry without auto_adjust
             print(f"Warning: Empty data for {ticker}. Retrying without auto_adjust...")
             df = yf.download(ticker, start=start, end=end, interval=interval, auto_adjust=False, progress=False)
             
        if df.empty:
            raise ValueError(f"Failed to fetch data for '{ticker}'. Symbol might be wrong or delisted.")

    except Exception as e:
        raise ValueError(f"Network/API error while fetching data: {e}")
    
    # Cleaning
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.droplevel(1)
    
    df.index.name = "Date"
    if df.index.tz is not None:
        df.index = df.index.tz_localize(None)
        
    return df

def preprocess_custom_data(uploaded_file: Any) -> pd.DataFrame:
    """Standardizes uploaded CSVs."""
    try:
        df = pd.read_csv(uploaded_file)
        df.columns = df.columns.str.strip().str.lower()
        
        col_map = {
            "date": "Date", "time": "Date",
            "close": "Close", "adj close": "Close", "last": "Close",
            "open": "Open", "high": "High", "low": "Low", "volume": "Volume"
        }
        df = df.rename(columns=col_map)
        
        if "Date" in df.columns:
            df["Date"] = pd.to_datetime(df["Date"])
            df = df.set_index("Date")
        else:
            df.index = pd.to_datetime(df.index)
            
        df = df.sort_index()
        
        if "Close" not in df.columns:
            raise ValueError("CSV missing required 'Close' column.")
            
        for col in ["Open", "High", "Low"]:
            if col not in df.columns:
                df[col] = df["Close"]
        if "Volume" not in df.columns:
            df["Volume"] = 0
            
        return df
    except Exception as e:
        raise ValueError(f"Error processing file: {e}")

def add_features(df: pd.DataFrame) -> pd.DataFrame:
    """Generates technical indicators with adaptive windows for learning."""
    if df.empty:
        raise ValueError("Empty DataFrame provided for feature generation.")

    df = df.copy()
    n_rows = len(df)
    
    # Target Setup
    df["Tomorrow"] = df["Close"].shift(-1)
    df["Target"] = (df["Tomorrow"] > df["Close"]).astype(int)
    
    # Adaptive Parameters
    if n_rows < 100:
        ma_short, ma_long = 5, 20
        rsi_window, bb_window, atr_window = 7, 10, 7
    else:
        ma_short, ma_long = 20, 50
        rsi_window, bb_window, atr_window = 14, 20, 14

    # --- Indicators ---
    # Moving Averages
    df[f"MA{ma_long}"] = df["Close"].rolling(window=ma_long).mean()
    if n_rows > 250:
        df["MA200"] = df["Close"].rolling(window=200).mean()
    
    # RSI
    delta = df["Close"].diff()
    gain = (delta.where(delta > 0, 0)).rolling(window=rsi_window).mean()
    loss = (-delta.where(delta < 0, 0)).rolling(window=rsi_window).mean()
    rs = gain / loss
    df["RSI"] = 100 - (100 / (1 + rs))
    
    # MACD
    if n_rows > 35:
        exp1 = df["Close"].ewm(span=12, adjust=False).mean()
        exp2 = df["Close"].ewm(span=26, adjust=False).mean()
        df["MACD"] = exp1 - exp2
        df["Signal_Line"] = df["MACD"].ewm(span=9, adjust=False).mean()
    else:
        df["MACD"] = 0
        df["Signal_Line"] = 0
    
    # Bollinger Bands
    df["BB_Mid"] = df["Close"].rolling(window=bb_window).mean()
    std_dev = df["Close"].rolling(window=bb_window).std()
    df["BB_Upper"] = df["BB_Mid"] + (2 * std_dev)
    df["BB_Lower"] = df["BB_Mid"] - (2 * std_dev)
    
    # ATR
    high_low = df["High"] - df["Low"]
    high_close = np.abs(df["High"] - df["Close"].shift())
    low_close = np.abs(df["Low"] - df["Close"].shift())
    tr = pd.concat([high_low, high_close, low_close], axis=1).max(axis=1)
    df["ATR"] = tr.rolling(window=atr_window).mean()
    
    # Ratios & Custom Features
    df[f"Trend_MA{ma_long}"] = df["Close"] / df[f"MA{ma_long}"]
    if "MA200" in df.columns:
        df["Trend_SMA200"] = df["Close"] / df["MA200"]
    
    # Safe Division due to potential zeros
    df["BB_Width"] = (df["BB_Upper"] - df["BB_Lower"]) / df["BB_Mid"].replace(0, 1)
    
    # Lags
    if n_rows > 100:
        for lag in [1, 2, 5]:
            df[f"Lag_{lag}"] = df["Close"] / df["Close"].shift(lag)
    
    # Cleanup
    df_clean = df.dropna()
    
    # Failsafe for small data
    if df_clean.empty and n_rows > 15:
        df_clean = df.fillna(method='bfill').fillna(method='ffill').dropna()

    if df_clean.empty:
        raise ValueError(f"Effective data too small after indicator calculation. Input: {n_rows} rows.")
        
    return df_clean

def create_sequences(data: np.ndarray, target: pd.Series, seq_length: int = 10) -> Tuple[np.ndarray, np.ndarray]:
    xs, ys = [], []
    for i in range(len(data) - seq_length):
        x = data[i:(i + seq_length)]
        y = target.iloc[i + seq_length] 
        xs.append(x)
        ys.append(y)
    return np.array(xs), np.array(ys)

def train_predict_deep(df: pd.DataFrame, predictors: List[str], model_arch: str = "LSTM", target_col: str = "Target", seq_len: int = 10, epochs: int = 50, random_state: int = 42) -> Tuple[nn.Module, float, pd.Series, pd.Series, pd.DataFrame, List[str]]:
    set_global_seed(random_state)
    
    if torch.cuda.is_available():
        device = torch.device('cuda')
    elif torch.backends.mps.is_available():
        device = torch.device('mps')
    else:
        device = torch.device('cpu')
    
    # Scaling
    from sklearn.preprocessing import StandardScaler
    scaler = StandardScaler()
    scaled_features = scaler.fit_transform(df[predictors])
    
    # Split
    split_idx = int(len(df) * 0.8)
    train_data = scaled_features[:split_idx]
    test_data = scaled_features[split_idx:]
    
    train_target = df[target_col].iloc[:split_idx]
    test_target = df[target_col].iloc[split_idx:]
    
    X_train, y_train = create_sequences(train_data, train_target, seq_len)
    X_test, y_test = create_sequences(test_data, test_target, seq_len)
    
    if len(X_train) == 0 or len(X_test) == 0:
        raise ValueError("Data too small for sequence generation.")

    X_train_t = torch.tensor(X_train, dtype=torch.float32).to(device)
    y_train_t = torch.tensor(y_train, dtype=torch.float32).unsqueeze(1).to(device)
    X_test_t = torch.tensor(X_test, dtype=torch.float32).to(device)
    
    if model_arch == "GRU":
        model = GRUClassifier(input_dim=len(predictors)).to(device)
    else:
        model = LSTMClassifier(input_dim=len(predictors)).to(device)
        
    criterion = nn.BCELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
    
    model.train()
    for _ in range(epochs):
        optimizer.zero_grad()
        outputs = model(X_train_t)
        loss = criterion(outputs, y_train_t)
        loss.backward()
        optimizer.step()
        
    model.eval()
    with torch.no_grad():
        probs = model(X_test_t).cpu().squeeze().numpy()
        
    preds = (probs > 0.5).astype(int)
    
    # Index Alignment
    actual_test_idx = df.index[split_idx + seq_len:]
    actual_test_idx = actual_test_idx[:len(preds)]
    
    preds_series = pd.Series(preds, index=actual_test_idx)
    probs_series = pd.Series(probs, index=actual_test_idx)
    precision = precision_score(df.loc[actual_test_idx, target_col], preds, zero_division=0)
    
    return model, precision, preds_series, probs_series, df.loc[actual_test_idx], predictors

def train_predict(df: pd.DataFrame, model_type: str = "Random Forest", random_state: int = 1, enable_tuning: bool = False) -> Tuple[Any, float, pd.Series, pd.Series, pd.DataFrame, List[str]]:
    """
    Main training interface supporting standard and deep learning models.
    """
    n_rows = len(df)
    
    # Data Splitting Logic
    # Modified to use 20% split for medium datasets (was fixed at 100) to improve evaluation robustness
    if n_rows > 2000: 
        split_idx = -252 # 1 Year for very large datasets
    else: 
        split_idx = -int(n_rows * 0.2) # 20% for everything else
    
    # Safety floor
    if abs(split_idx) < 5: split_idx = -5 
    
    train = df.iloc[:split_idx]
    test = df.iloc[split_idx:]
    
    # Dynamic Predictor Selection
    potential_cols = [
        "Close", "Volume", "Open", "High", "Low", "MA50", "MA200", 
        "RSI", "MACD", "BB_Upper", "BB_Lower", "ATR",
        "Trend_SMA50", "Trend_SMA200", "BB_Width", "Vol_Ratio",
        "Lag_1", "Lag_2", "Lag_5"
    ]
    predictors = [c for c in potential_cols if c in df.columns]
    
    # --- Deep Learning Branch ---
    if "LSTM" in model_type and "Voting" not in model_type:
        return train_predict_deep(df, predictors, model_arch="LSTM")
    if "GRU" in model_type:
        return train_predict_deep(df, predictors, model_arch="GRU")
        
    # --- Voting Ensemble ---
    # Combines predictions from:
    # 1. Random Forest (Robust decision trees)
    # 2. XGBoost (Gradient boosting for precision)
    # 3. LSTM (Deep Learning for sequential patterns)
    # The final probability is the average of all three.
    if model_type == "Voting (RF + XGB + LSTM)":
        # 1. Train RF
        rf_model, _, _, rf_probs, _, _ = train_predict(df, "Random Forest", random_state, enable_tuning)
        # 2. Train XGB
        xgb_model, _, _, xgb_probs, _, _ = train_predict(df, "XGBoost", random_state, enable_tuning)
        # 3. Train LSTM
        lstm_model, _, _, lstm_probs, lstm_test_df, _ = train_predict_deep(df, predictors, "LSTM")
        
        # Align Indices
        common_idx = rf_probs.index.intersection(lstm_probs.index)
        
        rf_p = rf_probs.loc[common_idx]
        xgb_p = xgb_probs.loc[common_idx]
        lstm_p = lstm_probs.loc[common_idx]
        
        final_probs = (rf_p + xgb_p + lstm_p) / 3
        final_preds = (final_probs > 0.5).astype(int)
        
        precision = precision_score(df.loc[common_idx, "Target"], final_preds, zero_division=0)
        
        # FIX: Return dataframe slice that matches the predictions exactly
        final_test_df = df.loc[common_idx]
        
        return "Voting Ensemble", precision, final_preds, final_probs, final_test_df, predictors

    # --- Standard ML Models ---
    if len(train) < 30:
         raise ValueError(f"Training set too small ({len(train)} rows). Expand date range.")
    
    n_pos = train["Target"].sum()
    n_neg = len(train) - n_pos
    scale_weight = n_neg / n_pos if n_pos > 0 else 1
    
    if model_type == "Random Forest":
        model = RandomForestClassifier(n_estimators=200, min_samples_split=50, random_state=random_state, class_weight="balanced")
    elif model_type == "XGBoost":
        model = XGBClassifier(n_estimators=200, max_depth=5, learning_rate=0.05, scale_pos_weight=scale_weight, eval_metric='logloss', random_state=random_state)
    elif model_type == "Logistic Regression":
        model = LogisticRegression(class_weight="balanced", random_state=random_state, max_iter=1000)
    elif model_type == "Stacked Ensemble":
        estimators = [
            ('rf', RandomForestClassifier(n_estimators=100, min_samples_split=50, random_state=random_state, class_weight="balanced")),
            ('xgb', XGBClassifier(n_estimators=100, max_depth=3, scale_pos_weight=scale_weight, eval_metric='logloss', random_state=random_state))
        ]
        model = StackingClassifier(estimators=estimators, final_estimator=LogisticRegression(), cv=3)

    model.fit(train[predictors], train["Target"])
    
    preds = model.predict(test[predictors])
    preds_series = pd.Series(preds, index=test.index)
    
    if hasattr(model, "predict_proba"):
        probs = model.predict_proba(test[predictors])[:, 1]
    else:
        probs = preds
        
    probs_series = pd.Series(probs, index=test.index)
    precision = precision_score(test["Target"], preds_series, zero_division=0)
    
    return model, precision, preds_series, probs_series, test, predictors

def calculate_monthly_returns(df: pd.DataFrame) -> pd.DataFrame:
    """Calculates month-end returns for heatmap."""
    if "Close" not in df.columns: return pd.DataFrame()
    
    if not isinstance(df.index, pd.DatetimeIndex):
         df.index = pd.to_datetime(df.index)
        
    monthly_data = df["Close"].resample("M").last()
    monthly_returns = monthly_data.pct_change() * 100
    
    returns_df = pd.DataFrame(monthly_returns).reset_index()
    returns_df.columns.values[0] = "Date"
    
    returns_df["Year"] = returns_df["Date"].dt.year
    returns_df["Month"] = returns_df["Date"].dt.month_name()
    returns_df["Month_Num"] = returns_df["Date"].dt.month
    
    heatmap_data = returns_df.pivot(index="Year", columns="Month_Num", values="Close")
    
    month_map = {1: "Jan", 2: "Feb", 3: "Mar", 4: "Apr", 5: "May", 6: "Jun", 
                 7: "Jul", 8: "Aug", 9: "Sep", 10: "Oct", 11: "Nov", 12: "Dec"}
    heatmap_data.columns = [month_map.get(c, c) for c in heatmap_data.columns]
    
    return heatmap_data
